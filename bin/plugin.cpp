...
bool parseOptionsAddHelp(boost::program_options::variables_map& vm, boost::program_options::options_description& desc, std::vector<std::string>& params) {
  bool help;
  desc.add_options()("help,c", boost::program_options::bool_switch(&help), "Print Help Message");

  try {
    boost::program_options::store(boost::program_options::command_line_parser(params).options(desc).run(), vm);
