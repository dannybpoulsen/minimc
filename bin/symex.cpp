...
namespace {
  ...
  public:
    ...
    MiniMC::Host::ExitCodes runCommand (MiniMC::Model::Program&& prgm, MiniMC::Support::Interaction& messager,const SetupOptions& sopt) {
      ...
      auto symbol = prgm.getRootFrame().resolve(locoptions.function);
      if (symbol) {
	MiniMC::Algorithms::GenCases::TestCaseGenerator generator {prgm};
	auto res = MiniMC::Support::AsyncExecutor{}.execute(messager,[&generator,&messager,&symbol,&sopt](){return generator.generate (messager.getMessager(),symbol.value(),sopt.smt.selsmt);});
